...
def run_game(*args, budget: int = 30, logger=lambda e: None) -> Dict[str, Any]:
    ...
    counters = {"repeated_page_attempts": 0, "budget_rejections": 0, "schema_violations": 0, "trap_detections": 0, "strategic_replans": 0, "fallback_used": 0, "api_errors": 0, "replan_count": 0, "escape_count": 0, "trap_count": 0, "fallback_used_count": 0}
    while state.steps_used < budget:
        ...
        counters["trap_count"] += int(meta.get("trap_detections", 0))
        counters["fallback_used_count"] += int(meta.get("fallback_used", 0))
        counters["escape_count"] += int(meta.get("escape_count", 0))
        if move is None:
        ...
        if strategy.should_replan(state):
            counters["replan_count"] += 1
            counters["strategic_replans"] += 1
